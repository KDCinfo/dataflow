...
function setClumpListFromStorageUsingSettings() {
  const clumpListKey = getStorageNameFromSettings();
  clumpList = JSON.parse(localStorage.getItem(clumpListKey) || '[]');
}

function getStorageNameFromSettings() {
  return settings.storageNames[settings.storageIndex] || 'dataFlowFallbackKey';
}
...
const lastAddedColDefault = 1;
let lastAddedCol = lastAddedColDefault;
...
const lastAddedClumpIdDefault = 0;
let lastAddedClumpId = lastAddedClumpIdDefault;
